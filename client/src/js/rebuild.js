(function (w) {
    ...
    $lhy.jsonp = function (options) {
        ...
        w.jsonpCallBack = function (res) {
            ...
            delete w.jsonpCallBack;
        }
        // 出错处理
        scriptNode.onerror = function () {
            container.removeChild(scriptNode);
            w.jsonpCallBack && delete w.jsonpCallBack;
        }
        scriptNode.type = 'text/javascript';
        container.appendChild(scriptNode);
    };
    $lhy.getAjax = function (url, fn) {
        function createXHR() {
            try {
...
function goLive() {
    ...
    new Promise(function (resolve, reject) {
        ...
        $lhy.getAjax(lifeUrl, getAjaxData);
    }).then(function (data) {
        var routerView = document.getElementById('router-view');
        routerView.innerHTML = data;
        Movie.dom = document.getElementById('db-cont');
        $lhy.jsonp({
            ...
            callback: function (res) {
                var movData = res;
                //对数据进行处理，去除{}里面的字符串内容
                Movie.showImg(Movie.dom, movData);
            }
        })
    });
}

var router = new $lhy.Router();
